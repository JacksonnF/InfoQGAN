...
def graph_distribution_financial(distribution):
    ...
    ) / 2  # Average of edges to find center
...
def generate_stock_dataset():
    ...
    aligned_returns = pd.concat(
        [aapl_returns, tsla_returns], axis=1, join="inner"
    ).dropna()
...
class DistributionDataset(Dataset):
    def __init__(self, datasets):
        ...
        """
        self.distributions = torch.tensor(
            datasets, dtype=torch.float32
        )  # Probability distributions as torch tensor
